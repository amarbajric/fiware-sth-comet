...
function aggregateNative(request, reply) {
    sthDatabase.getCollection(
        {
            service: request.headers[sthConfig.HEADER.FIWARE_SERVICE],
            servicePath: request.headers[sthConfig.HEADER.FIWARE_SERVICE_PATH],
        ...
        function (err, collection) {
            sthDatabase.getAvgAggregationData({
                collection: collection,
                entityId: request.params.entityId,
                entityType: request.params.entityType,
                attrName: request.params.attrName,
...
/**
 * Takes a start date and end date as string and returns the 'average' date (date between two dates)
 * (@IPPR impl.)
 * @param startDateStr
 * @param endDateStr
 * @returns {Date}
 */
function getAvgDate(startDateStr, endDateStr) {
    const startDate = new Date(startDateStr),
        endDate = new Date(endDateStr),
        diffMs = parseInt(endDate - startDate),
        avgDate = new Date();
    avgDate.setTime(startDate.getTime() + (diffMs/2));
    return avgDate;
};

/**
 * Returns aggregated data in avg format (@IPPR impl.)
 * @param data
 * @param dateFrom
 * @param dateTo
 * @param hLimit
 * @returns Array<NGSIPayloadData>
 */
function calculateAvgData(data, dateFrom, dateTo, hLimit) {
    if (hLimit >= data.length) {
        return data;
    } else {
        // TODO: Fix calculation of batchSize (hLimit should always be fulfilled or at least tried to)
        const dataCount = data.length,
              batchSize = (dataCount/hLimit) << 0,
              aggregatedData = [];

        data.reverse().reduce((acc, currVal, i, arr) => {
            if ((i % batchSize === 0 || i === (dataCount - 1)) && i > 0) {
                if (i === (dataCount -1)) {
                    i = dataCount;
                    acc += Number(currVal.attrValue);
                }
                const time = getAvgDate(arr[i - batchSize].recvTime, arr[i - 1].recvTime);
                aggregatedData.push({recvTime: time, attrType: currVal.attrType, attrValue: (acc / batchSize)});
                return Number(currVal.attrValue);
            } else {
                return acc + Number(currVal.attrValue);
            }
        }, 0);
...
/**
 * Returns historical aggregated average data to the client
 * @param {Object}   request The request
 * @param {Function} reply   Hapi's reply function
 */
function getAvgAggregatedData(request, reply) {
    var response;

    sthLogger.debug(
        request.sth.context,
        'Aggregating data and calculating average data points for given hLimit...'
    );
                    ...
                    function (err, result, totalCount) {
                        if (err) {
                            ...
                            if (result instanceof stream) {
                                ...
                                // Calculate avg of data points and limit it according to hLimit while aggregating
                                let aggrRes;
                                aggrRes = calculateAvgData(result, request.query.dateFrom, request.query.dateTo, request.query.hLimit);
                                sthLogger.debug(
                                    request.sth.context,
                                    'Responding with %s docs',
                                    aggrRes.length
...
function getDataHandler(request, reply) {
  ...
  if (request.query.hLimit && (request.query.aggrMethod && request.query.aggrMethod === 'avg')) {
      // Average aggregation
      // getAvgAggregatedData(request, reply);
      aggregateNative(request, reply);
  } else if ((request.query.lastN || request.query.lastN === 0) ||
