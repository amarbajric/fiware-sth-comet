...
function getMultiRawData(request, reply) {
    ...
    function generateMultiResponse() {
        Object.keys(collectionData).forEach(entityId => {
            if (request.query.nongsi) {
                const contextElement = sthServerUtils.getSingleNonNGSIResult(entityId, request.params.entityType,
        ...
        response = reply(request.query.nongsi ? sthServerUtils.getNonNGSIFormatPayload(contextElements) :
    ...
    function processRawData(err, result, totalCount, entityId, attrName) {
        ...
        if (err) {
            ...
            if (request.query.nongsi) {
                ...
                const attributePayload = sthServerUtils.getSingleAttributeResult(attrName, Object.keys(result[0]), keylessResultSet);
    ...
    function processCollectionHandler(err, collection, entityId) {
        if (err) {
            ...
            if (request.query.nongsi) {
                const emptyResult = sthServerUtils.getSingleNonNGSIResult(entityId, request.params.entityType, []);
