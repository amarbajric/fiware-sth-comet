...
function getMultiRawData(request, reply) {
    ...
    function generateMultiResponse() {
        Object.keys(collectionHandlers).forEach(entityId => {
            const contextElement = sthServerUtils.getMultiAttributesContextElement(entityId, request.params.entityType,
                collectionHandlers[entityId]['attributes']);
            contextElements.push(contextElement);
        });

        response = reply(sthServerUtils.getNGSIMultiContextElementsPayload(contextElements));
        sthServerUtils.addFiwareCorrelator(request, response);
    ...
    function processRawData(err, result, totalCount, entityId, attrName) {
        ...
        if (err) {
            ...
            sthLogger.error(
                request.sth.context,
                'Error when getting raw data from collection \'%s\'',
                collectionHandlers[entityId].collection
            );
            sthLogger.debug(
                request.sth.context,
                'Responding with 500 - Internal Error'
            ...
            collectionHandlers[entityId]['attributes'].push(emptyAttributePayload);
            ...
            collectionHandlers[entityId]['attributes'].push(attributePayload);
        }
        if (request.query.count) {
            sthLogger.debug(
                request.sth.context,
    ...
    function getCollectionData(entityId) {
        attrNames.forEach(attrName => {
            sthDatabase.getRawData(
                {
                    collection: collectionHandlers[entityId]['collection'],
                    entityId: entityId,
                    entityType: request.params.entityType,
                    attrName: attrName,
                    lastN: request.query.lastN,
    ...
    function processCollectionHandler(err, collection, entityId) {
        if (err) {
            ...
            const emptyContextElement = sthServerUtils.getMultiAttributesContextElement(entityId, request.params.entityType, []);
            ...
            collectionHandlers[entityId]['collection'] = collection;
            collectionHandlers[entityId]['attributes'] = [];
...
function getDataHandler(request, reply) {
  ...
  if ((request.query.lastN || request.query.lastN === 0) ||
    ...
    // Average aggregation
      mapReduceAggregation(request, reply);
