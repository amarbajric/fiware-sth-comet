...
function getMultiRawData(request, reply) {
    ...
    const profiler = {composeResponse: 0};
    entityIds.forEach(a => {
        profiler[a] = {};
        profiler[a]['collObj'] = 0;
        attrNames.forEach(e => profiler[a][e] = {getRawData: 0, processRawData: 0});
    });

    function getElapsedTime(startTime) { return new Date().getTime() - startTime; }
    ...
    function generateMultiResponse() {
        ...
        profiler['composeResponse'] = getElapsedTime(startTime);
        console.log(JSON.stringify(profiler));
    ...
    function processRawData(err, result, totalCount, entityId, attrName) {
        ...
        profiler[entityId][attrName]['processRawData'] = getElapsedTime(startTime);
    ...
    function getCollectionData(entityId) {
        ...
        attrNames.forEach(attrName => {
            sthDatabase.getRawData(
                ...
                // cb function
                (err, result, totalCount) => {profiler[entityId][attrName]['getRawData'] = getElapsedTime(startTime) ;processRawData(err, result, totalCount, entityId, attrName)}
            );
        });
    }
    ...
    entityIds.forEach(entityId => {
        ...
        sthDatabase.getCollection(
            ...
            // cb function
            (error, collection) => {profiler[entityId]['collObj'] = getElapsedTime(startTime) ; processCollectionHandler(error, collection, entityId)}
        );
    });
}
...
function getDataHandler(request, reply) {
  ...
  if ((request.query.lastN || request.query.lastN === 0) ||
        ((request.query.hLimit || request.query.hLimit === 0) &&
            ...
            (!request.query.aggrMethod && !request.query.aggrPeriod)) ||
    ...
    if (!request.params.filetype && !request.query.aggrPeriod && !request.query.aggrMethod) {
      ...
      (request.query.aggrMethod && request.query.aggrPeriod && !request.query.lastN)) {
      if (request.params.entityId.split(',').length > 1 || request.params.attrName.split(',').length > 1) {
