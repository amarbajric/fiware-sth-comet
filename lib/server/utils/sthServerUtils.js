 ...
 */
function aggregationGroupKeyBuilder(aggrPeriod) {
    let modifier;
    const groupKeyModulo = {'$toDate': {'$subtract': [{ '$toLong': '$recvTime' }, { '$mod':
                    [{ '$toLong': '$recvTime' }, 1000 * 60 * modifier] }]}};
    const groupKeySplitDate = JSON.parse('{"$' + modifier + '": "$recvTime"}');

    switch (aggrPeriod.toString()) {
        case 'quarterhour':
            modifier = 15;
            return groupKeyModulo;
        case 'halfhour':
            modifier = 30;
            return groupKeyModulo;
        case 'hour':
