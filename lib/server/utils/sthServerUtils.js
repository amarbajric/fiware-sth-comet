...
/**
 * @IPPR - AggregationGroupKeyBuilder
 * THis method builds the groupKey for the AggregationPipelineBuilder method
 * @param aggrPeriod string (e.g. 'hour', 'halfhour', 'month') => see sthServer.js aggrPeriod query parameter
 * @returns Object (The actual groupKey which is used for aggregation)
 */
function aggregationGroupKeyBuilder(aggrPeriod, timeZoneOffsetMillis) {
    const offsetMillis = !timeZoneOffsetMillis ? 0 : timeZoneOffsetMillis;
    const groupKeyModulo = (mod) => {return {'$toDate': {'$subtract': [{ '$toLong': '$recvTime' }, { '$mod':
                    [{ '$toLong': '$recvTime' }, 1000 * 60 * mod] }]}}; };
    const groupKeySplitDate = (mod) => JSON.parse('{"$' + mod + '": {"$add": ["$recvTime", ' + offsetMillis + ']}}');

    switch (aggrPeriod.toString()) {
        case 'quarterhour':
            return groupKeyModulo(15);
        case 'halfhour':
            return groupKeyModulo(30);
        case 'hour':
            return groupKeyModulo(60);
        case 'day':
            return groupKeySplitDate('dayOfYear');
        case 'week':
            return groupKeySplitDate('week');
        case 'month':
            return groupKeySplitDate('month');
        case null || undefined:
            return '$attrName'; // in case of no aggrPeriod, use attrName to aggregate over it
    }
}
 ...
 * @param aggrMethod string (e.g. 'avg', 'min', 'max')
 * @param groupKey Object (use the sthServerUtils.aggregationGroupKeyBuilder to build a proper groupKey)
 ...
 */
function aggregationPipelineBuilder(aggrMethod, groupKey, hLimit, skip, attrName, dateFrom, dateTo, timeZoneOffsetMillis) {
    ...
    */
    const offsetMillis = !timeZoneOffsetMillis ? 0 : timeZoneOffsetMillis;
    const aggregationObj = JSON.parse('{"$' + aggrMethod + '": "$value"}');
    const aggregationPipeline = [
        ...
        {'$group':
                {
                    '_id': '$groupKey',
                    'firstDate': {'$min': { '$add' : [ '$recvTime', offsetMillis ] }},
                    'attrValue': aggregationObj
                }
        },
        {'$sort': { '_id': 1 } }
    ];
    // removes the {$limit: hLimit} pipeline
    !hLimit ? aggregationPipeline.splice(1, 1) : null;
    // removes the {$skip: skip} pipeline
    !skip ? (!hLimit ? aggregationPipeline.splice(1, 1) : aggregationPipeline.splice(2, 1)) : null;
...
module.exports = {
    ...
    aggregationGroupKeyBuilder: aggregationGroupKeyBuilder,
};
