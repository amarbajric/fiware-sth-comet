...
function getAggregatedDataV2(data, callback) {
    ...
    const mapFunctionDate = function() {
        let key = new Date(0);
        if (aggrPeriod === 'month') {
            key = new Date(new Date(this.recvTime).setUTCDate(1)).setUTCHours(0, 0, 0, 0);
        } else if (aggrPeriod === 'day') {
            key = new Date(this.recvTime).setUTCHours(0, 0, 0, 0);
        } else if (aggrPeriod === 'hour') {
            key = new Date(this.recvTime).setUTCMinutes(0, 0, 0);
        }
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: key,
