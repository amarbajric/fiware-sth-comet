 ...
 * @param callback
 */
function getAvgAggregatedData(data, callback) {
    var collection = data.collection,
        entityId = data.entityId,
        entityType = data.entityType,
        attrName = data.attrName,
    ...
    let totalCount, batchSize, acc, id;
    ...
    const createDateKey = (period, recvTime) => {
        if (period === 'month') {
            return new Date(new Date(recvTime).setUTCDate(1)).setUTCHours(0, 0, 0, 0);
        } else if (period === 'day') {
            return new Date(recvTime).setUTCHours(0, 0, 0, 0);
        } else if (period === 'hour') {
            return new Date(recvTime).setUTCMinutes(0, 0, 0);
        }
    }

    const mapFunctionDate = function() {
        /*console.log('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
        console.log(data.aggrMethod);
        console.log(data.aggrPeriod);*/
        const key =  new Date(this.recvTime).setUTCMinutes(0, 0, 0);
        //const key =  new Date(this.recvTime).setUTCHours(0, 0, 0, 0);
        //const key =  new Date(new Date(this.recvTime).setUTCDate(1)).setUTCHours(0, 0, 0, 0);
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: key,
            attrType: this.attrType,
    ...
    const beautifyResultSet = (results) => {
    ...
    collection.find(findCondition)
        ...
        //.then(() => data.collection.mapReduce(mapFunction, reduceFunction, options))
        .then(() => data.collection.mapReduce(mapFunctionDate, reduceFunction, optionsDate))
        .then(results => beautifyResultSet(results))
        .then(modRes => callback ? process.nextTick(callback.bind(null, null, modRes, null)) : null)
...
module.exports = {
  ...
  getAvgAggregatedData: getAvgAggregatedData,
