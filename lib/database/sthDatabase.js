...
function getAggrDataFromNativeAggregationPipeline(data, callback) {
    const collection = data.collection,
        ...
        from = data.from,
        to = data.to;
        console.log("Correct (1/3)!");
        const timeZoneOffsetMillis = sthServerUtils.timeZoneOffsetMillis(from, 'Europe/Vienna');
        const aggrGroupKey = sthServerUtils.aggregationGroupKeyBuilder(aggrPeriod);
    ...
    console.log("Correct (2/3)!");
        collection.aggregate(aggrPipeline).toArray(function(err, results) {
            results.forEach(res => {
                ...
                } else if (aggrMethod === 'halfhour' || aggrPeriod === 'quarterhour') {
                    res['recvTime'] = new Date(res['firstDate'].getTime().setUTCSeconds(0, 0));
                }
