 ...
 * @param callback
 */
function getAggregatedDataV2(data, callback) {
    ...
    const timeZoneOffsetMillis = function(utcDateTime) {
        ...
        const locale = utcDate.toLocaleString('de-AT', { timeZone: 'Europe/Vienna' });
        const offset = new Date(locale).getUTCHours() - utcDate.getUTCHours();
        if (offset < 0) {
          return (24 - offset) * 36000000;
        } else {
          return offset * 36000000;
        }
    };
    ...
    let totalCount, batchSize, acc, id, timeZoneOffset;
    ...
    timeZoneOffset = timeZoneOffsetMillis(from);

    const mapFunction = function () {
        const key = id;
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: new Date(new Date(this.recvTime).getTime() + timeZoneOffset),
    ...
    const mapFunctionDate = function () {
        ...
        if (aggrPeriod === 'month') {
            ...
            key = new Date(new Date(this.recvTime).getTime() + timeZoneOffset).setHours(0, 0, 0, 0);
        } else if (aggrPeriod === 'hour') {
            key = new Date(new Date(this.recvTime).getTime() + timeZoneOffset).setMinutes(0, 0, 0);
        }
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: key,
    ...
    const finalizeFunction = function (key, reducedValue) {
        ...
        reducedValue.recvTime = new Date(reducedValue.totalTime / reducedValue.count);
        return reducedValue;
    };
    ...
    collection.find(findCondition)
        ...
        .then(count => {
            totalCount = count;
            if (!aggrPeriod) {
                batchSize = (totalCount / hLimit) << 0;
                options.scope.sizeDataset = totalCount;
                options.scope.batchSize = batchSize;
                return {mapFunc: mapFunction, opts: options};
            } else {
                return {mapFunc: mapFunctionDate, opts: optionsDate};
            }
        })
        .then(conf => data.collection.mapReduce(conf.mapFunc, reduceFunction, conf.opts))
...
module.exports = {
    ...
    getAggregatedDataV2: getAggregatedDataV2,
