...
function getAggrDataFromNativeAggregationPipeline(data, callback) {
    ...
    const timeZoneOffsetMillis = aggrPeriod ? sthServerUtils.timeZoneOffsetMillis(from, 'Europe/Vienna') : null;

    const aggrGroupKey = sthServerUtils.aggregationGroupKeyBuilder(aggrPeriod, timeZoneOffsetMillis);
    const aggrPipeline = sthServerUtils.aggregationPipelineBuilder(aggrMethod, aggrGroupKey, hLimit, hOffset,
        attrName, from, to, timeZoneOffsetMillis);
    ...
    collection.aggregate(aggrPipeline).toArray(function (err, results) {
        results.forEach(res => {
            ...
            } else if (aggrPeriod === 'day' || aggrPeriod === 'week') {
            ...
            } else if (aggrPeriod === 'halfhour' || aggrPeriod === 'quarterhour') {
                const timeFrameRound = aggrPeriod === 'halfhour' ? 1000 * 60 * 30 : 1000 * 60 * 15;
                res['recvTime'] = new Date(Math.round(res['firstDate'].getTime() / timeFrameRound) * timeFrameRound);
            }
 ...
 * @param callback
 */
function getAggrDataFromMapReduce(data, callback) {
    const collection = data.collection,
        entityId = data.entityId,
        entityType = data.entityType,
        attrName = data.attrName,
    ...
    let totalCount,
        batchSize,
        acc = 0, // represents the accumulator for the map function
    ...
    const mapFunction = function () {
        const key = id;
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: this.recvTime.getTime(),
            attrType: this.attrType,
            count: 1
        };
        emit(key, value);
        acc++;
        acc % batchSize === 0 ? id++ : null;
    };

    const mapFunctionDate = function () {
        let key;
        if (aggrPeriod === 'month') {
            key = new Date(new Date(this.recvTime.getTime() + timeZoneOffset).setDate(1)).setHours(0, 0, 0, 0);
        } else if (aggrPeriod === 'day') {
            key = new Date(this.recvTime.getTime() + timeZoneOffset).setHours(0, 0, 0, 0);
        } else if (aggrPeriod === 'hour') {
            key = new Date(this.recvTime.getTime() + timeZoneOffset).setMinutes(0, 0, 0);
        }
        const value = {
            totalValue: Number(this.attrValue),
            totalTime: key,
    ...
    const reduceFunction = function (key, values) {
        ...
        values.forEach(value => {
            if (aggrMethod === 'avg') {
                reducedObject.totalValue += value.totalValue;
            } else if (aggrMethod === 'min') {
                reducedObject.totalValue = reducedObject.totalValue === 0 ? value.totalValue :
                    (value.totalValue < reducedObject.totalValue ? value.totalValue : reducedObject.totalValue);
            } else if (aggrMethod === 'max') {
                reducedObject.totalValue = reducedObject.totalValue === 0 ? value.totalValue :
                    (value.totalValue > reducedObject.totalValue ? value.totalValue : reducedObject.totalValue);
            }
            reducedObject.totalTime += value.totalTime;
            reducedObject.count += value.count;
            reducedObject.attrType = value.attrType;
        });
    ...
    const finalizeFunction = function (key, reducedValue) {
        if (reducedValue.count > 0) {
            if (aggrMethod === 'avg') {
                reducedValue.attrValue = reducedValue.totalValue / reducedValue.count;
            } else if (aggrMethod === 'min' || aggrMethod === 'max') {
                reducedValue.attrValue = reducedValue.totalValue;
            }
        }
        // change the local timestamp back to utc by subtracting the offset (only if aggrPeriod is used)
        // this is needed because the frontend is doing utc to localtime conversion on its own
        reducedValue.recvTime = new Date((reducedValue.totalTime / reducedValue.count) - (aggrPeriod ? timeZoneOffset : 0));
        return reducedValue;
    };
    ...
    const optionsBuilder = function() {
        ...
        if (!aggrPeriod) {
            const additionalScope = {id: id, acc: acc, batchSize: batchSize};
            options.scope = {...options.scope, ...additionalScope};
        };
        return options;
    };

    const callMapReduce = function() {
        const mapFunctionOpt = !aggrPeriod ? mapFunction : mapFunctionDate;
        const mapReduceOpts = optionsBuilder();
        return data.collection.mapReduce(mapFunctionOpt, reduceFunction, mapReduceOpts);
    };
    ...
    const checkForOffset = (results) => {
        if (hOffset && hOffset > 0) {
            return results.splice(0, hOffset);
        } else {
            return results;
        }
    };
    ...
    collection.find(findCondition)
        .count()
        .then(count => {totalCount = count; batchSize = (totalCount / hLimit) << 0;})
        .then(() => callMapReduce())
        .then(makeNGSICompliant)
        .then(checkForOffset)
        .then(modRes => callback ? process.nextTick(callback.bind(null, null, modRes, totalCount)) : null)
        .catch(err => callback ? process.nextTick(callback.bind(null, err, null, totalCount)) : null);
}
