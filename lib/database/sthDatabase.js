...
function getAggregatedDataV2(data, callback) {
    ...
    const finalizeFunction = function (key, reducedValue) {
        ...
        // change the local timestamp back to utc by subtracting the offset
        // this is needed because the frontend is doing utc to localtime conversion on its own
        reducedValue.recvTime = new Date((reducedValue.totalTime / reducedValue.count) - timeZoneOffset);
        return reducedValue;
    };
